 ...
 */
// miniguava: Modified so much to remove unnecessary features. Refer MapMaker.java.
class MapMakerInternalMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
   ...
   */
  static final int MAXIMUM_CAPACITY = InternalUtils.MAX_POWER_OF_TWO;
      ...
      try {
        ...
        for (ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
          ...
          if (e.getHash() == hash
            ...
            boolean isExplicitlyRemoved;
            if (entryValue != null) {
              isExplicitlyRemoved = true;
  ...
  public int size() {
    ...
    return InternalUtils.saturatedCastToInt(sum);
  }
